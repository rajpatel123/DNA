...
/**
 * Created by vrajput on 11/14/2018.
 */

public class DotIndicatorSeekBar extends View {
    ...
    private DotOnSeekChangeListener mSeekChangeListener;
    private Rect mRect;
    ...
    private DotSeekParams mSeekParams;//save the params when seeking change.
    //seek bar
    ...
    //indicator
    private DotIndicator mIndicator;//the pop window above the seek bar
    ...
    public DotIndicatorSeekBar(Context context) {
        this(context, null);
    }

    public DotIndicatorSeekBar(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public DotIndicatorSeekBar(Context context, AttributeSet attrs, int defStyleAttr) {
     ...
     * .build();
     */
    DotIndicatorSeekBar(Builder builder) {
    ...
    /**
     *  Created Method for Dynamic tick count
     * @param progressArr
     */
    public void setDynamicTicks(int[] progressArr){
        mTicksCount=progressArr.length;
        if (mTicksCount != 0) {
            mTickMarksX = new float[progressArr.length];
            if (mShowTickText) {
                mTextCenterX = new float[progressArr.length];
                mTickTextsWidth = new float[progressArr.length];
            }
            mProgressArr = new float[progressArr.length];
            for (int i = 0; i < mProgressArr.length; i++) {
                mProgressArr[i] = progressArr[i];
            }
        }
    }
...
//        drawTickTexts(canvas);
    ...
    private int secondaryPercentage=0;


    public void setSecondaryProgress(int percentage){
        int width = mMeasuredWidth - mPaddingRight - mPaddingLeft;
        secondaryPercentage = mPaddingLeft + (width * percentage)/100;
    ...
    private void drawSecondaryProgress(Canvas canvas){
        if(secondaryPercentage >0 && secondaryPercentage>mProgressTrack.right){
            mStockPaint.setColor(Color.GRAY);
            mStockPaint.setStrokeWidth(mBackgroundTrackSize);
            canvas.drawLine(mProgressTrack.right, mProgressTrack.top, secondaryPercentage, mProgressTrack.bottom, mStockPaint);
        }
    }
...
//        }
    ...
    /**
     *  Values for showing only TICKS
     */
    private int tickCountPercentage =0;
    private float[] tickCountList = null;

    /**
     * Set Ticks     *
     * @param arr
     */
    public void setTicksPercentages(ArrayList<Integer> arr){
        tickCountList =new float[arr.size()];
        int width = mMeasuredWidth - mPaddingRight - mPaddingLeft;

        for(int index=0;index < arr.size();index++){
            tickCountList[index] = mPaddingLeft + (width * arr.get(index))/100;
        }
        tickCountPercentage = tickCountList.length;

        invalidate();
    }

    private float testGetThumbPosOnTickFloat() {
        if (tickCountPercentage != 0) {
    ...
    private void drawTickMarks(Canvas canvas) {
        ...
        float thumbCenterX = getThumbCenterX();
        for (int i = 0; i < tickCountList.length; i++) {
//            float thumbPosFloat = testGetThumbPosOnTickFloat();
            if (mTickMarksSweptHide) {
                if (thumbCenterX >= tickCountList[i]) {
                    continue;
                }
            }
            if (mTickMarksEndsHide) {
                if (i == 0 || i == tickCountList.length - 1) {
                    continue;
                }
            }
            if (i == getThumbPosOnTick() && tickCountPercentage > 2 && !mSeekSmoothly) {
                continue;
            }
//            if (i <= thumbPosFloat) {
//                mStockPaint.setColor(Color.RED);
//            } else {
//                mStockPaint.setColor(Color.YELLOW);
//            }

            if(tickCountList[i] > getThumbCenterX()){
                mStockPaint.setColor(Color.RED);
            }else{
                mStockPaint.setColor(Color.RED);
            }
...
//            } else
            ...
            int rectWidth = SizeUtils.dp2px(mContext, 3);
            ...
            canvas.drawRect(tickCountList[i],
                    mProgressTrack.top - dividerTickHeight / 2.0f,
                    tickCountList[i]+ rectWidth,
    ...
    private void initThumbBitmap() {
        ...
        if (mThumbDrawable instanceof Drawable) {
...
//            int stateCount = listDrawable.getStateCount();
    ...
    private DotSeekParams collectParams(boolean formUser) {
        if (mSeekParams == null) {
            mSeekParams = new DotSeekParams(this);
        }
     ...
     */
    public DotOnSeekChangeListener getOnSeekChangeListener() {
     ...
     */
    public void setOnSeekChangeListener(@NonNull DotOnSeekChangeListener listener) {
