...
public class RemotePDFViewPager extends ViewPager implements DownloadFile.Listener {
    ...
    public RemotePDFViewPager(Context context, String pdfUrl, DownloadFile.Listener listener, Map<String, String> extraHeaders) {
        super(context);
        this.context = context;
        this.listener = listener;

        init(new DownloadFileUrlConnectionImpl(context, new Handler(), this), pdfUrl,extraHeaders);
    }
    ...
    private void init(DownloadFile downloadFile, String pdfUrl, Map<String, String> extraHeaders) {
        setDownloader(downloadFile);
        downloadFile.download(pdfUrl,extraHeaders,
    ...
    private void init(AttributeSet attrs) {
        if (attrs != null) {
            ...
            if (pdfUrl != null && pdfUrl.length() > 0) {
                init(new DownloadFileUrlConnectionImpl(context, new Handler(), this), pdfUrl, null);
            }
